...
async function main() {
    ...
    const seriesList = await Promise.all(editorialSeriesBlueprints.map(ensureSeries));
...
- 后续可以逐步演进，而不是一次重写`,
    ...
    for (const item of posts) {
        ...
        const series = item.seriesSlug ? seriesBySlug.get(item.seriesSlug) : null;
            ...
            existing
                ? await prisma.post.update({
                    ...
                    data: {
                        ...
                        seriesOrder: series ? item.seriesOrder || null : null,
                    },
                })
    ...
    for (const item of editorialProjectPatches) {
