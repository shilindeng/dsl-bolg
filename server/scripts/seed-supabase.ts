...
async function main() {
    console.log('🌱 开始通过 Supabase Client 播种数据...');

    // Helper for upserting by slug
    async function upsert(table: string, matchCol: string, matchVal: string, data: any) {
        // Try select
    ...
    try {
        ...
        for (const p of postsData) {
            ...
            const post = await upsert('Post', 'slug', postData.slug, postData);
